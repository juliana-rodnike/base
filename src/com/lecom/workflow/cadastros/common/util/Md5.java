/** como utilizar esta classe MD5;
 *
 *	Md5 senha = new Md5("teste");  // string a ser criptografada
 *	senha.getDigest();
 *	System.out.println ("Saida: " + senha.getStringDigest());  // saida
 *
 */
...
public class Md5 {
	...
	public byte[] processString () {
		...
		try {
			...
			logger.warn("WF - Md5 - processString() - exception ignored", ex); //$NON-NLS-1$
		}
		throw new RuntimeException (this.getClass().getName()
				+ "[processString]"
	...
	public Md5 (String input, String enc) {
		...
		try {
			...
			logger.error("WF - Md5 - Md5(String input=" + input + ", String enc=" + enc + ")", e); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
