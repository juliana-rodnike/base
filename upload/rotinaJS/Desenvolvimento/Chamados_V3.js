	...
	if(codigoEtapa == REGISTRAR_CHAMADO){	
		...
		Form.fields("SETOR").subscribe("CHANGE", function(itemId, data, response) {
			...
			if(auxUniOrig == "99" && auxUnidade == "99"){ 
				...
				console.log("Chamado interno UAD");
		...
		// Alteração de subcategoria, validar aprovação do gerente e grupos do crédito
...
function setForm(){
	...
	if(codigoEtapa == ASSUMIR_NOVO || codigoEtapa == ASSUMIR_ANDAMENTO){ 
		...
		// Atualizar categoria e subcategoria para chamados encaminhados
		if(codigoEtapa == ASSUMIR_ANDAMENTO) {
			...
			console.log("aux cat: " + auxCat);
			console.log("aux sub: " + auxSub);
...
function atualizarModelo(response){
	...
	if(response == "Consórcio / Consignado / Previ"){ model = "99_cons_consig";    }
